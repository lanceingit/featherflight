...
Att_Est_Q::Att_Est_Q(SENSORS* sensor, LINK_MAVLINK* link_mavlink) :
	Estimator(sensor),
...
bool Att_Est_Q::init()
{
	...
	esprintf(buf, "acc0:%.3f 1:%.3f 2:%.3f", (double)_accel(0),
					(double)_accel(1),(double)_accel(2));
	//	LINK_DEBUG(buf);
//	esprintf(buf, "%x:acc0:%.3f 1:%.3f 2:%.3f ", (double)_sensors->inertialSensor->get_acc_x(),
//					(double)_sensors->inertialSensor->get_acc_y(),
//					(double)get_sensor()->inertialSensor->get_acc_z());

	esprintf(buf, "[att]%x %d", _sensors, _use_compass);
	LINK_DEBUG(buf);


//	Vector<3> acc(_sensors->inertialSensor->get_acc_x(), _sensors->inertialSensor->get_acc_y(), _sensors->inertialSensor->get_acc_z());
//	_accel = acc;
//	_accel(0) = copter.sensors.inertialSensor->get_acc_x();
//	_accel(1) = copter.sensors.inertialSensor->get_acc_y();
//	_accel(2) = copter.sensors.inertialSensor->get_acc_z();
...
void Att_Est_Q::run()
{
	...
	{
	//	_gyro(0) = _lp_gyro_x.apply(_sensors->inertialSensor->get_gyro_x());
	//	_gyro(1) = _lp_gyro_y.apply(_sensors->inertialSensor->get_gyro_y());
	//	_gyro(2) = _lp_gyro_z.apply(_sensors->inertialSensor->get_gyro_z());
	...
	//	_accel(0) = _lp_accel_x.apply(_sensors.inertialSensor->get_acc_x());
	//	_accel(1) = _lp_accel_y.apply(_sensors.inertialSensor->get_acc_y());
	//	_accel(2) = _lp_accel_z.apply(_sensors.inertialSensor->get_acc_z());
		_accel.data[0] = _sensors->inertialSensor->get_acc_x();
		_accel.data[1] = _sensors->inertialSensor->get_acc_y();
		_accel.data[2] = _sensors->inertialSensor->get_acc_z();
	...
	if(_use_compass)
	{
//		_mag(0) = _sensors->compass->get_mag_x();
//		_mag(1) = _sensors->compass->get_mag_y();
//		_mag(2) = _sensors->compass->get_mag_z();
		_mag.data[0] = _sensors->compass->get_mag_x();
		_mag.data[1] = _sensors->compass->get_mag_y();
		_mag.data[2] = _sensors->compass->get_mag_z();
